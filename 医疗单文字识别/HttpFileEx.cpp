...
//104：服务器处理失败
//500：异常错误
DWORD CHttpFileEx::PostDataMethod(LPCTSTR strDataSend, int DataSize, wchar_t **pRecvData, DWORD &RecvSize,int SendCode,int RecvCode)	
{
	...
	try
	{
...
#endif
			//发送
			if (pFile->SendRequest(strHeader,m_wpWideChar,m_nWideCharLen) == FALSE)
			{
				return 103;
			}
		}
		...
		if (RecvCode == 1)
		{
			...
			*pRecvData = (LPTSTR)m_wpWideChar;
			...
			//RecvSize = m_nWideCharLen;
			MultiToWide(strResponse.c_str(), strResponse.length());
			*pRecvData = (wchar_t*)m_wpWideChar;
			RecvSize = m_nWideCharLen;
#endif
		}
		//接收采用UTF8编码
...
//104：服务器处理失败
//500：异常错误
DWORD CHttpFileEx::PostPicDataMethod(CString strLocalPath, LPCTSTR strFormName, wchar_t **pRecvData, DWORD &RecvSize, int SendCode, int RecvCode)
{
	DWORD	dwRetCode = 100;
	...
	try
	{
		...
		if (SendCode == 1)
		{
			...
			//获取尾部数据转为UTF8后的长度
			dwPostSize = dwPicSize + m_nWideCharLen + dwPostEndSize;
			//发送，告知数据总长度
			pFile->SendRequestEx(dwPostSize);
			pFile->Write(m_wpWideChar,m_nWideCharLen);
		}
		//发送时选择UTF8
		else if (SendCode == 2)
		{
...
DWORD CHttpFileEx::PostPicDataMethodForMat(cv::Mat srcImg, LPCTSTR strFormName, wchar_t **pRecvData, DWORD &RecvSize, int SendCode, int RecvCode)
{
	DWORD	dwRetCode = 100;
	DWORD	dwPostSize = 0;		//要传送的数据大小
	DWORD	dwPicSize = 0;	//图片大小
	...
	//构造数据包
	MakeHeaderData();
	MakePostPicData(strFormName, "C:\\HLMarkOCX\\tempimg.jpg");	//在原有数据包后面加上图片前缀包
	MakeEndData();
	//数据包构造完毕
	...
	try
	{
		...
		if (dwPicSize > 0)
		{
			int n = dwPicSize / BuffSize;
			int m = dwPicSize%BuffSize;
			if (m != 0)
				n++;
			for (int i = 0; i < n-1; i++)
			{
				memcpy(buffer, reinterpret_cast<char*>(&buff[BuffSize*i]), BuffSize);
				pFile->Write(buffer, BuffSize);
			}
			if (m != 0)
				BuffSize = m;
			memcpy(buffer, reinterpret_cast<char*>(&buff[BuffSize*(n-1)]), BuffSize);
			pFile->Write(buffer, BuffSize);
...
void CHttpFileEx::UTF8ToMultiByte(LPCSTR pUTF8, int UTF8Len)
{
	...
	////宽字符转为多字节
	//m_nWideCharLen = WideCharToMultiByte(CP_ACP,0,m_wpWideChar,m_nWideCharLen,NULL,0,NULL,NULL);
	//m_wpWideChar = new wchar_t[m_nWideCharLen + 1];
	//m_wpWideChar[m_nWideCharLen] = 0;
	//
	//WideCharToMultiByte(CP_UTF8,0,m_wpWideChar,m_nWideCharLen,m_wpWideChar,m_nWideCharLen,NULL,NULL);
}
//将宽字符转为UTF-8
...
void CHttpFileEx::WideToMulti(LPCWSTR wpWideData, int WideLen)
{
	...
	//m_nWideCharLen = WideCharToMultiByte(CP_ACP,0,wpWideData,WideLen,NULL,0,NULL,NULL);
	//m_wpWideChar = new wchar_t[m_nWideCharLen + 1];
	//m_wpWideChar[m_nWideCharLen] = '\0';

	//WideCharToMultiByte(CP_ACP,0,wpWideData,WideLen,m_wpWideChar,m_nWideCharLen,NULL,NULL);
}
...
void CHttpFileEx::MultiToMulti(LPCSTR strDataSend, int DataSize)
{
	...
	//}
	//
	//m_nWideCharLen = DataSize;
	//m_wpWideChar = new	char[m_nWideCharLen + 1];
	//m_wpWideChar[m_nWideCharLen] = '\0';

	//memcpy(m_wpWideChar,(LPCSTR)strDataSend,m_nWideCharLen);
}
...
//构造图片数据前缀包
void CHttpFileEx::MakePostPicData2(LPCTSTR strFormName, LPCTSTR Img)
{
	if (strFormName == NULL)
	{
		return;
